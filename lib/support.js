...
function scope(projectPath, moduleName, scopeName, filePatternFile) {
  let filePatterns = require(path.resolve(filePatternFile));
  const prefix = (!moduleName || moduleName == '') ? 'file:' : `file[${moduleName}]:`;
...
`<component name="DependencyValidationManager">
  ...
  try {
    fs.statSync(scopeDir);
  } catch(err) {
    fs.mkdirpSync(scopeDir);
  }
